...
export const accountClient = {
    ...
    updateAccount: async (account: Account) => {
        ...
        getRazorpayConnectDetails: async (accountId: IdParam) => {
        const response = await api.post<GenericDataResponse<RazorpayLinkedDetails>>(`accounts/${accountId}/razorpay/connect`);
        return response.data;
    },
    getRazorpayConnectAccounts: async (accountId: IdParam) => {
        const response = await api.get<GenericDataResponse<RazorpayLinkedAccountsResponse>>(`accounts/${accountId}/razorpay/linked_accounts`);
        return response.data;
    },
