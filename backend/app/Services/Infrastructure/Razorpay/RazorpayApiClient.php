...
class RazorpayApiClient implements RazorpayClientInterface
{
    ...
    {
        return (object)$this->api->request->request('POST', 'v2/accounts', $data);
    }

    public function fetchLinkedAccount(string $accountId): object
    {
        return (object)$this->api->request->request('GET', 'v2/accounts/' . $accountId);
    }

    public function createStakeholder(string $accountId, array $data): object
    {
        return (object)$this->api->request->request('POST', 'v2/accounts/' . $accountId . '/stakeholders', $data);
    }

    public function fetchStakeholder(string $accountId, string $stakeholderId): object
    {
        return (object)$this->api->request->request('GET', 'v2/accounts/' . $accountId . '/stakeholders/' . $stakeholderId);
    }

    public function updateStakeholder(string $accountId, string $stakeholderId, array $data): object
    {
        return (object)$this->api->request->request('PATCH', 'v2/accounts/' . $accountId . '/stakeholders/' . $stakeholderId, $data);
    }

    public function createProductConfiguration(string $accountId, array $data): object
    {
        return (object)$this->api->request->request('POST', 'v2/accounts/' . $accountId . '/products', $data);
    }

    public function fetchProductConfiguration(string $accountId, string $productId): object
    {
        return (object)$this->api->request->request('GET', 'v2/accounts/' . $accountId . '/products/' . $productId);
    }

    public function updateProductConfiguration(string $accountId, string $productId, array $data): object
    {
        return (object)$this->api->request->request('PATCH', 'v2/accounts/' . $accountId . '/products/' . $productId, $data);
    }
